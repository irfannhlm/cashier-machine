...
class TransFrame(Frame):
    def __init__(self, master=None, cnf={}, **kw):
        Widget.__init__(self, master, 'label', cnf, kw)
        super().__init__()
        self.configure(bg="#04b533")
        hwnd = self.winfo_id()
        colorkey = win32api.RGB(4, 181, 51)
...
main_window.iconbitmap(f"{filepath}/images/main_icon.ico")

bg_mainimg = ImageTk.PhotoImage(pil.open(f"{filepath}/images/bg_mainimg(adjusted).jpg").resize((1136, 640), pil.Resampling.LANCZOS))

bg_canvas = Canvas(main_window)
bg_canvas.pack(fill=BOTH, expand=1)
bg_canvas.create_image(0, 0, image=bg_mainimg, anchor=NW)
...
def loop1():
    global counter1, counter2, counter3, counter4, loopcount, bg1, bg2, resized_bg
    try:
        if button_products.winfo_exists():
         
            main_window.update()
            bg_canvas.delete(ALL)
            bg1 = pil.open(f"{filepath}/images/bg_mainimg(adjusted).jpg")
            resized_bg = bg1.resize((main_window.winfo_width(), main_window.winfo_height()), pil.Resampling.LANCZOS)
            bg2 = ImageTk.PhotoImage(resized_bg)
            bg_canvas.create_image(0, 0, image=bg2, anchor=NW)
            bg_canvas.create_text(main_window.winfo_width()/2, main_window.winfo_height() / 2 - frame_mainButtons.winfo_height(), 
                                  font=("Oretans Shadow", 80), text="STEI-MART!", fill="#fad3ac")
            ...
            if loopcount > 10:
                if main_window.winfo_width() < 680 or main_window.winfo_height() < 450:
...
def loop2():
    global counter1, counter2, loopcount, bg1, bg2, resized_bg
    try:
        if label_subTotal.winfo_exists():
    ...
    try:
        if button_print.winfo_exists():
            ...
            bg_canvas.create_text(main_window.winfo_width() / 2, main_window.winfo_height() / 2 - frame_endButtons.winfo_height() / 1.2, 
                                  font=("Oretans Shadow", 50), text="Terima Kasih Telah Berbelanja!", fill="#fad3ac")
            ...
            if loopcount > 10:
                if main_window.winfo_width() < 1040 or main_window.winfo_height() < 610:
                    if messagebox.showerror("Error Size Window", "Size window terlalu kecil!") == "ok":
                        main_window.geometry("1136x640"
                                             f"+{(main_window.winfo_screenwidth() - 1136) // 2}"
                                             f"+{(main_window.winfo_screenheight() - 640) // 2}")
...
"""
        ...
        window_info.iconbitmap(f"{filepath}/images/info_icon.ico")

        bg_img = ImageTk.PhotoImage(pil.open(f"{filepath}/images/bg_info.jpg").resize((568, 320), pil.Resampling.LANCZOS))
        window_info.bg_img = bg_img
        bgcanvas_info = Canvas(window_info)
        bgcanvas_info.pack(fill=BOTH, expand=1)

        bgcanvas_info.create_image(0, 0, image=bg_img, anchor=NW)
        bgcanvas_info.create_text(568/2, 20, font=("Maiandra GD", 15), width=568, text="Program Model Kasir Supermarket v1.2", justify=CENTER)
...
def open_help():
    ...
    if counter2 != 1:
        ...
        window_help.iconbitmap(f"{filepath}/images/help_icon.ico")

        window_help.update()
        bgcanvas_help = Canvas(window_help, bg="#fad8b6")
        ...
        bgframe2_help = Frame(bgcanvas_help, bg="#fad8b6")
        ...
        try:
            if button_products.winfo_exists():
                ...
                count= 0
                Label(bgframe2_help, font=("Maiandra GD", 20), text="Petunjuk Penggunaan Program", anchor=CENTER, bg="#fad8b6").grid(row=0, column=0, sticky=E + W, padx=5, pady=5)
                ...
                for ar in range(6):
                    if count == 0:
                        ...
                        teks1 = "Tombol Uang:"
                        teks2 = "Tekan tombol ini untuk melihat stok lembaran uang yang terdapat di kasir."
                    elif count == 5:    
                        teks1 = "Tombol Produk:"
                        teks2 = "Tekan tombol ini untuk melihat produk dan promo yang sedang berlaku."
                    
                    Label(bgframe2_help, font=("Maiandra GD", 15), text=teks1, anchor=CENTER, relief=RIDGE, padx=5, pady=5, bg="#fad8b6").grid(row=3 * ar + 1, column=0, sticky=E + W, padx=5)
                    Label(bgframe2_help, font=("Maiandra GD", 12), text=teks2, anchor=W, justify=LEFT, relief=RIDGE, padx=5, pady=5, bg="#fad8b6").grid(row=3 * ar + 2, column=0, sticky=E + W, padx=5)
                    Label(bgframe2_help, bg="#fad8b6").grid(row=3 * ar + 3, column=0)
                    count += 1
        except:
            pass
...
def window_mulai():
    ...
    frame_mainButtons = TransFrame(main_window)
...
def window_listproduk():
    ...
    if counter3 != 1:
        ...
        window_produk.iconbitmap(f"{filepath}/images/products_icon.ico")
        window_produk.update()
...
def window_listcash():
    ...
    if counter4 != 1:
        ...
        window_cash.iconbitmap(f"{filepath}/images/cash_icon.ico")
        window_cash.update()
        ...
        for r in range(0, len(jumlah_uang), 2):
            ...
            Label(bgframe2_cash, font=("Maiandra GD", 18), text=f"{jumlah_uang[list_uang[r]]} lembar",
                  padx=5, pady=2, relief=RIDGE).grid(row=1+3*(r//2), column=0, sticky=W+E+N+S, padx=10)
        ...
        for s in range(1, len(jumlah_uang), 2):
            ...
            Label(bgframe2_cash, font=("Maiandra GD", 18), text=f"{jumlah_uang[list_uang[s]]} lembar",
                  padx=5, pady=2, relief=RIDGE).grid(row=1+3*((s-1)//2), column=1, sticky=W+E+N+S, padx=10)
...
def opt_add_cash():
    ...
    for t in range(0, len(list_uang), 2):
        ...
        Label(bgframe2_addCash, font=("Maiandra GD", 18), text=f"{jumlah_uang[list_uang[t]]} lembar",
              padx=5, pady=2, width=10, relief=RIDGE).grid(row=1 + 3 * (t // 2), column=0, sticky=N+S)
    ...
    for u in range(1, len(list_uang), 2):
        ...
        Label(bgframe2_addCash, font=("Maiandra GD", 18), text=f"{jumlah_uang[list_uang[u]]} lembar",
              padx=5, pady=2, width=10, relief=RIDGE).grid(row=1 + 3 * ((u-1) // 2), column=4, sticky=N+S)
        Button(bgframe2_addCash, width=3, font=("Maiandra GD", 15), text="+",
...
def tampil_cash():
    ...
    for v in range(0, len(list_uang), 2):
        ...
        Label(bgframe2_cash, font=("Maiandra GD", 18), text=f"{jumlah_uang[list_uang[v]]} lembar",
              padx=5, pady=2, relief=RIDGE).grid(row=1 + 3 * (v // 2), column=0, sticky=W + E + N + S, padx=10)
    ...
    for w in range(1, len(list_uang), 2):
        ...
        Label(bgframe2_cash, font=("Maiandra GD", 18), text=f"{jumlah_uang[list_uang[w]]} lembar",
              padx=5, pady=2, relief=RIDGE).grid(row=1 + 3 * ((w - 1) // 2), column=1, sticky=W + E + N + S, padx=10)
...
def window_selesai():
    ...
    if produk_invoice == dict():
        ...
        frame_endButtons = TransFrame(main_window)
...
def save_invoice():
    invoicepath = filedialog.asksaveasfile(initialdir=f"{filepath}/receipt/", initialfile=f"{fname}.txt", defaultextension=".txt",
...
def print_invoice():
    ...
    if count_invoice != 1:
        ...
        with open(f"{filepath}/receipt/{fname}.txt", "w") as f:
            f.write(teks_invoice)
