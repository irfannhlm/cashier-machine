...
def loop1():
    ...
    try:
        if button_products.winfo_exists():
            main_window.update()
            button_exit.place(x=5, y=5)
            button_info.place(x=main_window.winfo_width() - button_info.winfo_width() - 5, y=5)
            button_help.place(x=5, y=main_window.winfo_height() - button_help.winfo_height() - 5)
            label_clock.place(x=main_window.winfo_width() - label_clock.winfo_width() - 5,
                              y=main_window.winfo_height() - label_clock.winfo_height() - 5)
            ...
            label_judul.place(x=(main_window.winfo_width() - label_judul.winfo_width()) // 2,
                              y=(main_window.winfo_height() - frame_mainButtons.winfo_height()) // 2 -
                              label_judul.winfo_height() * 1.5)
            ...
            if loopcount > 10:
                if main_window.winfo_width() < 450 or main_window.winfo_height() < 450:
    ...
    try:
        if label_subTotal.winfo_exists():
            ...
            button_back.place(x=main_window.winfo_width() // 2-button_next.winfo_width() - 50,
                              y=main_window.winfo_height() - button_back.winfo_height() - 5,
                              width=button_next.winfo_width())
            button_next.place(x=main_window.winfo_width() // 2 + 50,
                              y=main_window.winfo_height() - button_next.winfo_height() - 5)
    ...
    try:
        if frame_hitungKembalian.winfo_exists():
            ...
            frame_numpad.place(x=(frame2_hitungKembalian.winfo_width() - (frame_numpad.winfo_width() +
                               frame_kembalian.winfo_width() + 25)) / 2, y=frame_kembalian.winfo_y())
            frame_kembalian.place(x=(frame2_hitungKembalian.winfo_width() - (frame_numpad.winfo_width() +
                                  frame_kembalian.winfo_width() + 20)) / 2 + frame_numpad.winfo_width() + 18,
                                  y=((frame2_hitungKembalian.winfo_height() - (frame_tampilTunai.winfo_height() +
...
def loop4():
    ...
    try:
        if button_print.winfo_exists():
            ...
            label_judulAkhir.place(x=(main_window.winfo_width() - label_judulAkhir.winfo_width()) // 2,
                              y=(main_window.winfo_height() - frame_endButtons.winfo_height()) // 2 -
                              label_judulAkhir.winfo_height() * 1.5)
            label_clock.config(text=strftime("%H:%M WIB"))
            ...
            if loopcount > 10:
                if main_window.winfo_width() < 810 or main_window.winfo_height() < 610:
                    if messagebox.showerror("Error Size Window", "Size window terlalu kecil!") == "ok":
                        main_window.geometry("1136x640"
                                             f"+{(main_window.winfo_screenwidth() - 1136) // 2}"
                                             f"+{(main_window.winfo_screenheight() - 640) // 2}")
...
def open_info():
    ...
    if counter1 != 1:
        counter1 = 1
        teksinfo = f"""Program ini dibuat oleh Kelompok 3 Kelas 22 Pengenalan Komputasi dengan anggota:
...
Tujuan dibuatnya program ini adalah untuk memenuhi kewajiban Tugas Besar 1 dari mata
kuliah Pengenalan Komputasi yang diampu oleh Dr. Nur Ulfa Maulidevi, S.T., M.Sc.
"""
        ...
        Label(window_info, font=("Times New Roman", 15), text="Program Model Kasir Supermarket v1", anchor=CENTER,
              padx=10, justify=LEFT).grid(row=0, column=0, sticky=W+E, pady=7)
        Label(window_info, font=("Times New Roman", 12), text=teksinfo, anchor=NW,
              padx=10, justify=LEFT).grid(row=1, column=0, sticky=W+E)


def open_help():
    global counter2, window_help, bgframe_help, bgcanvas_help
    if counter2 != 1:
        counter2 = 1
        ...
        window_help.update()
        bgframe_help = LabelFrame(window_help)
        bgframe_help.pack(side=LEFT, fill=BOTH, expand=1)
        bgcanvas_help = Canvas(bgframe_help)
        bgcanvas_help.pack(side=LEFT, fill=BOTH, expand=1)
        scroll_help = Scrollbar(bgframe_help, orient=VERTICAL, command=bgcanvas_help.yview)
        scroll_help.pack(side=RIGHT, fill=Y)
        bgframe2_help = Frame(bgcanvas_help, padx=3, pady=3)
        ...
        try:
            if button_products.winfo_exists():
                for i in range(15):
                    Label(bgframe2_help, font=("Times New Roman", 15), text="Testing Stuffs").pack()
        except:
            pass
...
def window_mulai():
    global button_exit, button_info, button_help, label_clock, label_judul, frame_mainButtons, button_products
    global harga_total, produk_invoice, loopcount, count_invoice
    ...
    button_exit = Button(main_window, font=("Times New Roman", 14), text="Exit", padx=5, pady=5,
                         command=main_window.destroy)
    button_info = Button(main_window, font=("Times New Roman", 14), text="Info", padx=5, pady=5, command=open_info)
    button_help = Button(main_window, font=("Times New Roman", 14), text="Help", padx=5, pady=5, command=open_help)
    label_clock = Label(main_window, font=("Times New Roman", 14), relief=RAISED, padx=5, pady=5)
    button_exit.place()
    button_info.place()
    button_help.place()
    label_clock.place()
    ...
    label_judul = Label(main_window, font=("Times New Roman", 45), text="STEI-Mart", pady=10)
    button_start = Button(frame_mainButtons, font=("Times New Roman", 24), text="Mulai", width=7, command=window_harga)
    button_cash = Button(frame_mainButtons, font=("Times New Roman", 24), text="Uang", width=7, command=window_listcash)
    button_products = Button(frame_mainButtons, font=("Times New Roman", 24), text="Produk", width=7,
...
def window_listproduk():
    ...
    if counter3 != 1:
        ...
        button_randomize = Button(window_produk, font=("Times New Roman", 18), text="Randomize", command=randomize)
        button_listProduk = Button(window_produk, font=("Times New Roman", 18), text="Produk", command=tampil_produk)
        button_listDiskon = Button(window_produk, font=("Times New Roman", 18), text="Diskon", command=tampil_diskon)
        button_listPromo = Button(window_produk, font=("Times New Roman", 18), text="Promo", command=tampil_promo)
            ...
            try:
                if list_produk[d].diskon == 0.0:
                    temptext = f"{list_produk[d].nama}\nHarga: Rp{list_produk[d].harga:,.02f}\nKode Produk: " \
                               f"{list_produk[d].nama[0:3]}{list_produk[d].harga // 100}\nStock: " \
                               f"{list_produk[d].kuantitas}"
                    Label(bgframe2_produk, font=("Times New Roman", 14), text=temptext, anchor=CENTER, padx=5, pady=5,
            ...
            try:
                if list_produk[e].diskon == 0.0:
                    temptext = f"{list_produk[e].nama}\nHarga: Rp{list_produk[e].harga:,.02f}\nKode Produk: " \
                               f"{list_produk[e].nama[0:3]}{list_produk[e].harga // 100}\nStock: " \
            ...
            try:
                if list_produk[f].diskon == 0.0:
                    temptext = f"{list_produk[f].nama}\nHarga: Rp{list_produk[f].harga:,.02f}\nKode Produk: " \
                               f"{list_produk[f].nama[0:3]}{list_produk[f].harga // 100}\nStock: " \
            ...
            try:
                if list_produk[g].diskon == 0.0:
                    temptext = f"{list_produk[g].nama}\nHarga: Rp{list_produk[g].harga:,.02f}\nKode Produk: " \
                               f"{list_produk[g].nama[0:3]}{list_produk[g].harga // 100}\nStock: " \
            ...
            try:
                if list_produk[h].diskon == 0.0:
                    ...
                    temptext = f"{list_produk[h].nama}\nHarga: Rp\u0336" + "\u0336".join(
                               ...
                               f"{list_produk[h].nama[0:3]}{list_produk[h].harga // 100}\nStock: {list_produk[h].kuantitas}"
                    Label(bgframe2_produk, font=("Times New Roman", 14), text=temptext, anchor=CENTER, padx=5, pady=5,
            ...
            try:
                if list_produk[i].diskon == 0.0:
                    ...
                    temptext = f"{list_produk[i].nama}\nHarga: Rp\u0336" + "\u0336".join(
                               ...
                               f"{list_produk[i].nama[0:3]}{list_produk[i].harga // 100}\nStock: {list_produk[i].kuantitas}"
                    Label(bgframe2_produk, font=("Times New Roman", 14), text=temptext, anchor=CENTER, padx=5, pady=5,
...
def tampil_promo():
    ...
    try:
        ...
        for j in range(0, len(list_promo) + 1):
            ...
            Label(bgframe2_promo, font=("Times New Roman", 25), text=list_promo[j].nama,
                  anchor=W).grid(row=0 + 6 * j, column=0, sticky=W + E, padx=5, pady=3)
            Label(bgframe2_promo, font=("Times New Roman", 14), text=f"{teks_minProduk}",
                  anchor=W).grid(row=1 +6 * j, column=0, sticky=W + E, padx=5, pady=3)
            Label(bgframe2_promo, font=("Times New Roman", 14), text=f"Harga minimum: Rp{list_promo[j].minHarga:,.02f}",
                  anchor=W).grid(row=2 + 6 * j, column=0, sticky=W + E, padx=5, pady=3)
            Label(bgframe2_promo, font=("Times New Roman", 14),
                  text=f"Total promo: Rp{list_promo[j].reduksiHarga:,.02f}",
                  anchor=W).grid(row=3 + 6 * j, column=0, sticky=W + E, padx=5, pady=3)
            Label(bgframe2_promo, font=("Times New Roman", 14), text=f"Durasi Waktu: {teks_durasiWaktu}",
...
def window_listcash():
    ...
    if counter4 != 1:
        ...
        label_judulCash = Label(window_cash, font=("Times New Roman", 24), text="Jumlah Lembaran Uang", padx=5, pady=5)
        button_addCash = Button(window_cash, font=("Times New Roman", 18), text="Tambah Uang", command=opt_add_cash)
        button_tampilCash = Button(window_cash, font=("Times New Roman", 18), text="Tampil Jumlah", command=tampil_cash)
        ...
        for r in range(0, len(jumlah_uang), 2):
            Label(bgframe2_cash, font=("Times New Roman", 24), text=f"Rp{nilai_uang[list_uang[r]]:,.02f}",
                  padx=5, pady=2, width=13).grid(row=0+3*(r//2), column=0, sticky=W+E+N+S, padx=10)
            Label(bgframe2_cash, font=("Times New Roman", 18), text=f"{jumlah_uang[list_uang[r]]} lembar",
        ...
        for s in range(1, len(jumlah_uang), 2):
            Label(bgframe2_cash, font=("Times New Roman", 24), text=f"Rp{nilai_uang[list_uang[s]]:,.02f}",
                  padx=5, pady=2, width=13).grid(row=0+3*((s-1)//2), column=1, sticky=W+E+N+S, padx=10)
            Label(bgframe2_cash, font=("Times New Roman", 18), text=f"{jumlah_uang[list_uang[s]]} lembar",
...
def opt_add_cash():
    ...
    for t in range(0, len(list_uang), 2):
        Label(bgframe2_addCash, font=("Times New Roman", 24), text=f"Rp{nilai_uang[list_uang[t]]:,.02f}",
              padx=5, pady=2, width=13).grid(row=0 + 3 * (t // 2), column=0, columnspan=4, sticky=W + E + N + S,padx=10)
        Label(bgframe2_addCash, font=("Times New Roman", 18), text=f"{jumlah_uang[list_uang[t]]} lembar",
              padx=5, pady=2, width=10, relief=GROOVE).grid(row=1 + 3 * (t // 2), column=0, sticky=N+S)
        Button(bgframe2_addCash, width=3, font=("Times New Roman", 15), text="+",
    ...
    for u in range(1, len(list_uang), 2):
        Label(bgframe2_addCash, font=("Times New Roman", 24), text=f"Rp{nilai_uang[list_uang[u]]:,.02f}",
              padx=5, pady=2, width=13).grid(row=0 + 3 * ((u - 1) // 2), column=4, columnspan=4, sticky=W + E + N + S,
                                             padx=10)
        Label(bgframe2_addCash, font=("Times New Roman", 18), text=f"{jumlah_uang[list_uang[u]]} lembar",
...
def tampil_cash():
    ...
    for v in range(0, len(list_uang), 2):
        Label(bgframe2_cash, font=("Times New Roman", 24), text=f"Rp{nilai_uang[list_uang[v]]:,.02f}",
              padx=5, pady=2, width=13).grid(row=0 + 3 * (v // 2), column=0, sticky=W + E + N + S, padx=10)
        Label(bgframe2_cash, font=("Times New Roman", 18), text=f"{jumlah_uang[list_uang[v]]} lembar",
    ...
    for w in range(1, len(list_uang), 2):
        Label(bgframe2_cash, font=("Times New Roman", 24), text=f"Rp{nilai_uang[list_uang[w]]:,.02f}",
              padx=5, pady=2, width=13).grid(row=0 + 3 * ((w - 1) // 2), column=1, sticky=W + E + N + S, padx=10)
        Label(bgframe2_cash, font=("Times New Roman", 18), text=f"{jumlah_uang[list_uang[w]]} lembar",
...
def window_harga():
    ...
    button_back = Button(main_window, font=("Times New Roman", 14), text="Back to Main", padx=5, pady=5,
                         command=window_mulai)
    button_next = Button(main_window, font=("Times New Roman", 14), text="Hitung Kembalian", padx=5, pady=5,
    ...
    label_kodeProduk = Label(frame_kodeProduk, font=("Times New Roman", 15), text="Kode:", relief=RAISED, padx=6)
    entry_kodeProduk = Entry(frame_kodeProduk, font=("Times New Roman", 15), width=15)
    button_cekKode = Button(frame_kodeProduk, font=("Times New Roman", 15), text="Cek", command=cek_harga)
...
def hitung_harga():
    ...
    if kode_produk[entry_kodeProduk.get()].kuantitas >= int(jumlah_produk):
        ...
        # Simpan informasi pembelian untuk membuat invoice.
        if kode_produk[entry_kodeProduk.get()] not in produk_invoice:
            produk_invoice[kode_produk[entry_kodeProduk.get()]] = [int(jumlah_produk),
                                                                        kode_produk[entry_kodeProduk.get()].harga,
                                                                        kode_produk[entry_kodeProduk.get()
                                                                        ].harga * int(jumlah_produk)]
        else:
            produk_invoice[kode_produk[entry_kodeProduk.get()]][0] += int(jumlah_produk)
            produk_invoice[kode_produk[entry_kodeProduk.get()]][2] += kode_produk[entry_kodeProduk.get()
                                                                           ].harga * int(jumlah_produk)
...
def hitung_promo():
    ...
    for v in list_promo:
        ...
        for w in v.minProduk:
            ...
            # Jika pada setiap produk yang terdapat pada invoice pembelian memenuhi kriteria minProduk dari promo,
            # produk akan ditambahkan ke dalam set templist_cekPromo.
            if w in produk_invoice and produk_invoice[w][0] >= v.minProduk[w]:
...
def window_kembalian():
    ...
    if harga_total != 0:
        ...
        button_back = Button(main_window, font=("Times New Roman", 14), text="Kembali Hitung Harga", padx=5, pady=5,
                            command=window_harga)
        button_next = Button(main_window, font=("Times New Roman", 14), text="Selesai Transaksi", padx=5, pady=5,
        ...
        label_judulTunai = Label(frame_tunai, font=("Times New Roman", 15), text="Uang Tunai:", anchor=W, bg="black",
        ...
        label_judulHarga = Label(frame_harga, font=("Times New Roman", 15), text="Harga Total:", anchor=W, bg="black",
                                fg="white", relief=GROOVE, padx=5, pady=5)
        ...
        label_totalKembalian = Label(frame_kembalian, font=("Times New Roman", 18), textvariable=tampil_kembalian, anchor=W,
...
def hitung_kembalian():
    ...
    try:
        if int(uang_tunai) > harga_total:
            ...
            while kembalian_total >= 100:
                ...
                for aa in jumlah_uang:
                    if nilai_uang[aa] > nilai_uang[max_jumlahUang]:
                        continue
            ...
            for ad in range(0, len(templist_lembaran), 2):
                Label(frame_lembaran, font=("Times New Roman", 13), anchor=W,
        ...
        Label(frame_lembaran, font=("Times New Roman", 13), text="Stok lembaran uang tidak mencukupi!").grid(row=0, column=0)
...
def window_selesai():
    ...
    if produk_invoice == dict():
        ...
        label_judulAkhir = Label(main_window, font=("Times New Roman", 45), text="Terima Kasih Telah Berbelanja!", pady=10)
        button_restart = Button(frame_endButtons, font=("Times New Roman", 24), text="Back to Main", width=14,
                                command=window_mulai)
        button_save = Button(frame_endButtons, font=("Times New Roman", 24), text="Save Invoice", width=14,
                             command=save_invoice)
        button_print = Button(frame_endButtons, font=("Times New Roman", 24), text="Print Invoice", width=14,
                              command=print_invoice)
        frame_endButtons.place()
        label_judulAkhir.place()
...
def create_invoice():
    ...
    harga_jual = 0
    harga_diskon = 0
    for af in produk_invoice:
        harga_jual += produk_invoice[af][2]
        harga_diskon += int(af.harga * af.diskon * produk_invoice[af][0])
    ...
    for ag in produk_invoice:
        temphargaproduk = f"{produk_invoice[ag][1]:,}"
        temphargatotalproduk = f"{produk_invoice[ag][2]:,}"
        ...
        if maxjumlah is None:
            maxjumlah = len(str(produk_invoice[ag][0]))
        elif len(str(produk_invoice[ag][0])) > maxjumlah:
            maxjumlah = len(str(produk_invoice[ag][0]))
    ...
    for ai in produk_invoice:
        temphargaproduk = f"{produk_invoice[ai][1]:,}"
        temphargatotalproduk = f"{produk_invoice[ai][2]:,}"

        line_produk = f"{left(ai.nama, maxname)}   {right(str(produk_invoice[ai][0]), maxjumlah)}  " \
...
{right("Kembalian: ", len(dashline) - maxtotal)}{right(f"{tempkembalian}", maxtotal)}"""
    ...
    line4 = "TERIMA KASIH TELAH BELANJA DI STEI-MART!"
    ...
    if len(dashline) < len(line4):
        ...
        teks_produk = ""
        for ak in produk_invoice:
            temphargaproduk = f"{produk_invoice[ak][1]:,}"
            temphargatotalproduk = f"{produk_invoice[ak][2]:,}"

            line_produk = f"{left(ak.nama, maxname)}   {right(str(produk_invoice[ak][0]), maxjumlah)}  " \
                          f"{right(temphargaproduk, maxharga)}  {right(temphargatotalproduk, maxtotal)}\n"
...
def save_invoice():
    invoicepath = filedialog.asksaveasfile(initialdir=filepath, initialfile=f"{fname}.txt", defaultextension=".txt",
                                           filetypes=[("Text File", "*txt")])
    if invoicepath is None:
        return

    with open(invoicepath.name, "w") as f:
        f.write(teks_invoice)
...
def center(teks, length):
    if len(teks) <= length:
        tempteks = " "*((length-len(teks))//2) + f"{teks}"
        return tempteks
